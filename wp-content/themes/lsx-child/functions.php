...
{
  if(!empty($_FILES["logo-0"]["tmp_name"]))
  {
  	$images = count($_FILES) - 1;
  	for ($i=0; $i < $images; $i++) { 
	    $urls = wp_handle_upload($_FILES["logo-".$i], array('test_form' => FALSE));
	    $temp[] = $urls["url"];
	}
    return $temp;  
  }
   ...
   ?>
   		<script type="text/javascript" language="JavaScript">
		    ...
		    jQuery(document).ready(function() {           
		        jQuery("#add").click(function() {
		            var elementRow = jQuery("#logo").clone();
		            var newId = "logo-" + elementCounter;
		            
		            elementRow.attr("id", newId);
		            elementRow.show();
		                
		            var inputField = jQuery("input", elementRow);
		            inputField.attr("name", "logo-" + elementCounter); 

		            var name = "logo-" + elementCounter;
		            ...
		            jQuery("#add-image").append(elementRow);
		...
		<div class="wrap">
		    ...
		    <div id="logo" style="display:none;">
		        ...
		        <?php
		         if(get_option('<script>document.write(name)</script>') != ''){ ?>
		        	<img width="40px" height="40px" style="margin:-30px;" src="<?php echo get_option('<script>document.write(name)</script>'); ?>">
		        <?php } ?>
		            <input type="button" class="remove" name="remove" value="Remove Image">
		    </div>
		</div>
			<input type="file" name="logo-0"/>
	        <?php
	        $logos = get_option('logo');
	         if($logos != ''){ ?>
	        	<img width="40px" height="40px" style="margin:-30px;" src="<?php echo $logos[0]; ?>">
	        <?php } ?>	
	        <input type="button"  id="add" value="Add Image">  
	        <div id="add-image"></div> 
		    ...
		    <div id="logo">
		      	<input type="file" name="logo"/>
		        <img width="40px" height="40px" style="margin:-30px;" src="<?php echo $logo; ?>">
		        <input type="button" class="remove" name="remove" value="Remove Image">
		    </div>
		    <?php } ?>
		</div>
