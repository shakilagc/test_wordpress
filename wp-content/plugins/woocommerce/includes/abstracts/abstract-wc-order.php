...
abstract class WC_Abstract_Order {
	...
	public function get_item_downloads( $item ) {
		...
		$download_ids = $wpdb->get_results( $wpdb->prepare("
			SELECT download_id, download_count, downloads_remaining
	...
	public function display_item_downloads( $item ) {
		$product = WC_Abstract_Order::get_product_from_item( $item );

		if ( $product && $product->exists() && $product->is_downloadable() && $this->is_download_permitted() ) {
			$download_files = WC_Abstract_Order::get_item_downloads( $item );
			$i              = 0;
			$links          = array();
